...
- If the host starts executing a Goal Contract, it should maintain `goal-progress-tracker` with current execution state and keep the resulting `Goal Progress Log` as runtime metadata.
...
- Once the runtime begins execution, use `goal-progress-tracker` against `Goal Contract + current_status + completed_steps + attempted_paths + verified_evidence + active_blockers + resume_hint` and keep updating it at runtime checkpoints.
